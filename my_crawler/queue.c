...
void Queue_setValue(QueueElem* data, QueueElem temp) {
    ...
    memset(data->url,0,MAX_URL_LEN+1);
    strncpy(data->url,temp.url,strlen(temp.url));
}
...
void Queue_enqueue(QueueInfoPtr * const qinfo, QueueElem elem, int *error,TListInfoPtr * tinfo) {
  ...
  //  printf("Locking \n");
    ...
    TListElem telem;
    memset(telem.url, 0, MAX_URL_LEN);
    strncpy(telem.url, elem.url, strlen(elem.url));

    int found = 0;
    TListNodePtr foundNode = NULL;


    TList_find(*tinfo, telem, &foundNode, &found);

    if (found == 1) {
        //nothing
    }else if(strcmp(telem.url,"finito")==0){
        Queue_insert(qinfo, elem, error);
    }else {
        TList_insert(tinfo, telem, error);
        if (*error == 1) {
            printf("ERROR IN INSERTION IN TEMPLIST\n!");
        }
        Queue_insert(qinfo, elem, error);
    }
