...
RequestPtr Request_build(char * filepath,char * host, int port){
    RequestPtr rinfo = Request_create();
    
    rinfo->port = port;
    
    rinfo->filepath = malloc(sizeof(char)*strlen(filepath)+1);
    memset(rinfo->filepath,0,strlen(filepath)+1);
    strncpy(rinfo->filepath,filepath,strlen(filepath));
    
    rinfo->host = malloc(sizeof(char)*strlen(host)+1);
    memset(rinfo->host,0,strlen(host)+1);
    strncpy(rinfo->host,host,strlen(host));
