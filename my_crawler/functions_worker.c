...
int requestValidation(char * msg) {
    ...
    w = multi_tok(NULL, &save_ptr, "\r\n"); //third word must be last and HTTP/1.1 TODO: DEN LEITOURGEI LOGW /n/r thelei multi_tok
    printf("current word:---%s---  \n", w);
...
char * tokenize_url(char * url){
    ...
    char * urlCopy = (char*) malloc((urlLength + 1) * sizeof (char));
    memset(urlCopy, 0, (urlLength + 1));
    strncpy(urlCopy, url, urlLength);
    //printf("message length:%d", urlLength);
    //printf("\nUnedited url:%s\n", urlCopy);
    
    
    char * save_ptr = NULL;
    
    char * w = multi_tok(urlCopy, &save_ptr, "page");   //find page in url
    
   // printf("rest:---%s--- \n", save_ptr);
    
    w = strtok_r(save_ptr, "_" , &save_ptr);
    
    //printf("tokenized:---%s--- and rest:---%s--- \n", w,save_ptr);
    
    char * buffer = malloc(sizeof(char) * MAX_URL_LEN);
    memset(buffer,0,MAX_URL_LEN);
    
    sprintf(buffer,"/site%s/page%s_%s",w,w,save_ptr);
    
    //printf("constructed path is: %s with length %d \n",buffer ,strlen(buffer));
...
int parseResponseHeader(char * header, int * content_length){
    
    int headerLength = strlen(header);
    char * headerCopy = (char*) malloc((headerLength + 1) * sizeof (char));
    memset(headerCopy, 0, (headerLength + 1));
    strncpy(headerCopy, header, headerLength);
    
    char * save_ptr = NULL;
    
    char * w = strtok_r(headerCopy, " \t" , &save_ptr); //first word usually http protocol
    
    w = strtok_r(NULL," \t",&save_ptr); //second word is the response code and third description
    int code = atoi(w);
    
    while ((w = strtok_r(NULL, " \t\r\n", &save_ptr)) != NULL) {    //find content length in bytes
        //printf("Word:---%s---\n", w);
        if (strcmp(w, "Content-Length:") == 0) {
            if ((w = multi_tok(NULL, &save_ptr, " \r\n")) != NULL) {
                //printf("CONTENT LENGTH :%s \n",w);
                (*content_length) = atoi(w);
            }else{
                //printf("didnt find content length!\n");
            }
        }
    }
    
    //printf("Code is %s and content_length is :%d \n",w,content_length);
